...
<template>
    <DialogRoot v-model:open="open">
        ...
        >
            ...
            <kbd
                class="hidden rounded border border-gray-200 bg-gray-100 px-1.5 font-mono text-[10px] text-gray-500 sm:inline dark:border-white/10 dark:bg-white/5 dark:text-gray-400"
            >⌘K</kbd>
        </DialogTrigger>
        <DialogPortal>
            <DialogOverlay
            ...
            >
                ...
                <div class="flex items-center gap-3 border-b border-gray-200 px-4 py-3 dark:border-white/10">
                    ...
                    />
                    <DialogClose
                        class="rounded border border-gray-200 px-1.5 font-mono text-[10px] text-gray-500 transition hover:text-gray-900 dark:border-white/10 dark:text-gray-400 dark:hover:text-white"
                        aria-label="Close"
                    >ESC</DialogClose>
                </div>
                ...
                <ul class="max-h-[60vh] overflow-y-auto p-2">
                    ...
                    <li v-for="entry in results" :key="entry.title + entry.href">
                        <Link
                            :href="entry.href"
                            class="flex items-start justify-between gap-3 rounded-lg px-3 py-2.5 text-left transition hover:bg-gray-100 dark:hover:bg-white/5"
                            @click="open = false"
                        >
                            <div class="min-w-0">
                                <p class="truncate text-sm font-medium text-gray-900 dark:text-white">{{ entry.title }}</p>
                                <p class="truncate text-xs text-gray-500 dark:text-gray-400">{{ entry.body }}</p>
                            </div>
                            <span
                                class="shrink-0 rounded-full border border-gray-200 px-2 py-0.5 text-[10px] uppercase tracking-wider text-gray-500 dark:border-white/10 dark:text-gray-400"
                            >{{ entry.section }}</span>
                        </Link>
                    </li>
                </ul>
